...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        # Initialize variables for tracking best weights and objective value
        best_weights = f.weights.copy()
        best_val = np.inf if self.out_type_ == 'best' else None

        # Initialize variables for tracking average weights
        avg_weights = np.zeros(f.weights.shape[0])
        ...
        while t < self.max_iter_ and delta >= self.tol_:
            ...
            delta_weights = eta * grad
            new_weights = f.weights - delta_weights

            # Compute Euclidean norm of weight update
            delta = np.linalg.norm(delta_weights)
            ...
            # Update best weights and objective value if necessary
            if self.out_type_ == 'best' and val < best_val:
                best_weights = f.weights.copy()
                best_val = val
        ...
        # Compute the final solution based on the out_type_
        if self.out_type_ == 'last':
            return f.weights
        elif self.out_type_ == 'best':
            return best_weights
        else:  # elif self.out_type_ == 'average':
