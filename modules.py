...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.array([np.linalg.norm(self.weights, 2)])
...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.array([np.linalg.norm(self.weights, 1)])
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # return -(1 / m) * np.sum(y * X.T @ self.weights - np.log(1 + np.exp(X.T @ self.weights)))
        z = np.dot(X, self.weights)
        yz = y * z
        return -np.mean(yz - np.log(1 + np.exp(z)))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # return X.T @ (y - 1 / (1 + np.exp(-X.T @ self.weights)))
        # m = X.shape[1]
        # z = np.dot(X, self.weights)
        # grad = -np.dot(X.T, y - (np.exp(z) / (1 + np.exp(z)))) / m
        # return grad
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.array(
            [self.fidelity_module_.compute_output(**kwargs) + self.lam_ * self.regularization_module_.compute_output(
                **kwargs)])
